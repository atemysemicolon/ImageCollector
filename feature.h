...
//    void forwardPass(const cv::Mat &img);
    ...
    std::vector<cv::Mat> forwardPassRescaleImg(const cv::Mat &img,int layer_nr);
...
std::vector<cv::Mat> CaffeModel::forwardPassRescaleImg(const cv::Mat &img, int layer_nr)
{
    Blob<float>* input_layer = net_->input_blobs()[0];
    cv::Size input_size = cv::Size(input_layer->width(),input_layer->height());
    //std::cout<<"Input size : "<<input_size<<std::endl<<std::endl;
